...
export function scenario_1() {
  ...
  group(`page_2 - http://${__ENV.HOSTNAME}:3389/shop?product=CarChargeBox&name=${users[(vu.idInTest - 1) % users.length].username}`, function () {
    response = http.post(
      `http://${__ENV.HOSTNAME}:3389/shop?product=CarChargeBox&name=${users[(vu.idInTest - 1) % users.length].username}`,
      {
  ...
  group(`page_4 - http://${__ENV.HOSTNAME}:3389/shop?product=HeatPump&name=${users[(vu.idInTest - 1) % users.length].username}`, function () {
    response = http.post(
      `http://${__ENV.HOSTNAME}:3389/shop?product=HeatPump&name=${users[(vu.idInTest - 1) % users.length].username}`,
      {
        product: 'HeatPump',
      },
      {
  ...
  group(`page_7 - http://${__ENV.HOSTNAME}:3389/shop?product=Internet&name=${users[(vu.idInTest - 1) % users.length].username}`, function () {
    response = http.post(
      `http://${__ENV.HOSTNAME}:3389/shop?product=Internet&name=${users[(vu.idInTest - 1) % users.length].username}`,
      {
        product: 'Internet',
      },
      {
  ...
  group(`page_8 - http://${__ENV.HOSTNAME}:3389/shop?product=SolarPanel&name=${users[(vu.idInTest - 1) % users.length].username}`, function () {
    response = http.post(
      `http://${__ENV.HOSTNAME}:3389/shop?product=SolarPanel&name=${users[(vu.idInTest - 1) % users.length].username}`,
      {
        product: 'SolarPanel',
      },
      {
  ...
  group(`page_9 - http://${__ENV.HOSTNAME}:3389/shop?product=HeatPump&name=${users[(vu.idInTest - 1) % users.length].username}`, function () {
    response = http.post(
      `http://${__ENV.HOSTNAME}:3389/shop?product=HeatPump&name=${users[(vu.idInTest - 1) % users.length].username}`,
      {
        product: 'HeatPump',
  ...
  group(`page_11a - http://${__ENV.HOSTNAME}:3389/shop?product=HeatPump&name=${users[(vu.idInTest - 1) % users.length].username}`, function () {
    response = http.post(
